...
if ! ipset list illegal_crime_mc_blacklist &>/dev/null; then
	echo "Creating ipset illegal_crime_mc_blacklist..."
	ipset create illegal_crime_mc_blacklist hash:net maxelem 1048575
fi

echo "Adding IPs to ipset illegal_crime_mc_blacklist..."
while read -r ip; do
	if is_ipv4 "$ip"; then
		ipset add -! illegal_crime_mc_blacklist "$ip"
	fi
done < "$SORTED_FILE"

echo "Updateing iptables rules"
if ! iptables -C DOCKER-USER -m set --match-set illegal_crime_mc_blacklist src -d 10.100.1.0/24 -j LOG --log-prefix "IP Blacklisted DOCKER-USER" &>/dev/null; then
    iptables -I DOCKER-USER 1 -m set --match-set illegal_crime_mc_blacklist src -d 10.100.1.0/24 -j LOG --log-prefix "IP Blacklisted DOCKER-USER"
fi
if ! iptables -C DOCKER-USER -m set --match-set illegal_crime_mc_blacklist src -d 10.100.1.0/24 -j DROP &>/dev/null; then
    iptables -I DOCKER-USER 2 -m set --match-set illegal_crime_mc_blacklist src -d 10.100.1.0/24 -j DROP
fi
